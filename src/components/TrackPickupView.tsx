    ...
    <div className="relative min-h-screen py-10 sm:py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-12">
      ...
      <div className="text-center max-w-4xl mx-auto space-y-5 relative z-10">
        ...
        <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black font-heading text-zinc-900 tracking-tight leading-none">
          {language === 'en' ? (
            <>Track Your <span className="text-[#15803D]">Dead Battery</span> Fleet</>
          ) : (
      ...
      {selectedRecord && (
        <div className="space-y-8 max-w-6xl mx-auto relative z-10 animate-in fade-in-50 slide-in-from-bottom-6 duration-300">
          ...
          <div className="liquid-glass rounded-3xl overflow-hidden shadow-2xl border border-emerald-500/20">
              ...
              <div className="relative rounded-2xl overflow-hidden border-2 border-emerald-500/30 bg-[#07130B] shadow-2xl">
                ...
                <div className="absolute bottom-3 left-3 right-3 flex flex-wrap items-center justify-between gap-3 bg-[#0A160E]/90 backdrop-blur-md p-3 sm:p-4 rounded-xl border border-emerald-500/30 text-white">
                  ...
                  <div className="flex items-center gap-3 sm:gap-5 text-xs font-mono">
                    ...
                    <div className="flex items-center gap-2">
                      <a
                        href={`tel:${selectedRecord.driverPhone || '01819556721'}`}
                        className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-sans font-bold text-xs flex items-center gap-1.5 transition-transform hover:scale-105 shadow-md ring-2 ring-emerald-400/50"
                      >
                        <Phone className="w-3.5 h-3.5" />
                        <span>Call Truck Driver: {selectedRecord.driverPhone || '+880 1819-556721'}</span>
                      </a>
                      <a
                        href={`tel:${selectedRecord.agentPhone || '01711884210'}`}
                        className="px-3 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-sans font-semibold text-xs flex items-center gap-1 transition-transform hover:scale-105 border border-zinc-700"
                      >
                        <Phone className="w-3 h-3 text-emerald-400" />
                        <span>Agent: {selectedRecord.agentPhone || '+880 1711-884210'}</span>
                      </a>
                    </div>
                  </div>
                </div>
            ...
            {/* DEDICATED ASSIGNED TRUCK & DRIVER CONTACT BANNER */}
            <div className="p-4 sm:p-5 bg-linear-to-r from-emerald-950 via-zinc-900 to-[#0A160E] text-white border-b border-emerald-800/40 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-3.5">
                <div className="relative">
                  <div className="w-12 h-12 rounded-2xl bg-emerald-800/80 border border-emerald-400/40 flex items-center justify-center text-white font-bold text-lg font-heading shadow-md">
                    KU
                  </div>
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-heading font-black text-sm sm:text-base text-white">
                      {selectedRecord.driverName || 'Kamal Uddin'}
                    </span>
                  ...
                  <div className="text-xs text-zinc-300 flex items-center gap-2 mt-0.5">
                    <span className="font-mono text-emerald-400 font-semibold">{selectedRecord.vehicleNumber || 'Dhaka Metro-U 14-3892'}</span>
                    <span>•</span>
                    <span className="text-zinc-400">Carrier Unit #04</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <a
                  href={`tel:${selectedRecord.driverPhone || '01819556721'}`}
                  className="px-4 py-2.5 rounded-xl bg-[#15803D] hover:bg-[#166534] text-white font-bold text-xs sm:text-sm flex items-center gap-2 shadow-lg transition-transform hover:scale-105 ring-2 ring-emerald-400/30"
                >
                  <Phone className="w-4 h-4 animate-pulse" />
                  <span>Call Driver ({selectedRecord.driverPhone || '+880 1819-556721'})</span>
                </a>
                <a
                  href={`https://wa.me/${(selectedRecord.driverPhone || '8801819556721').replace(/[^0-9]/g, '')}`}
                  target="_blank"
                  rel="noreferrer"
                  className="px-3.5 py-2.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-bold text-xs flex items-center gap-1.5 border border-zinc-700 transition-colors"
                >
            ...
            <div className="p-6 sm:p-8 bg-white/95 border-b border-zinc-200">
              ...
              {/* 5-STEPPING CHAIN OF CUSTODY (NO LINE - PURE ANIMATED CARDS) */}
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3.5 sm:gap-4">
                {[
                  ...
                  },
                  {
                    ...
                    sub: selectedRecord.driverName || 'Kamal Uddin',
                    icon: 'truck',
                  },
                  {
                    status: 'collected',
                  ...
                  return (
                    <div
                      key={stepItem.status}
                      className={`relative p-4.5 rounded-2xl border transition-all duration-500 transform hover:-translate-y-1 hover:shadow-lg ${
                        isDone
                          ? 'bg-linear-to-b from-emerald-50/90 to-emerald-100/40 border-emerald-400/80 shadow-xs ring-1 ring-emerald-500/20'
                          : isCurrent
                          ? 'bg-white border-[#15803D] ring-3 ring-emerald-500/30 shadow-xl scale-[1.03] animate-pulse-slow'
                          : 'bg-zinc-50/70 border-zinc-200 text-zinc-400 hover:border-zinc-300'
                      }`}
                    >
                      {/* Active Status Floating Badge */}
                      {isCurrent && (
                        <div className="absolute -top-2.5 right-3 px-2 py-0.5 rounded-full bg-[#15803D] text-white text-[9px] font-mono font-bold tracking-wider shadow-xs animate-bounce">
                          LIVE
                        </div>
                      )}
                      ...
                      <div className="flex items-center gap-2.5 mb-2.5">
                        <div
                          className={`w-8 h-8 rounded-xl flex items-center justify-center text-xs font-black shrink-0 transition-transform duration-300 ${
                            isDone
                              ? 'bg-[#15803D] text-white shadow-xs'
                              : isCurrent
                              ? 'bg-amber-500 text-white ring-4 ring-amber-200 animate-spin-slow'
                      ...
                      <p className={`text-[11px] leading-snug pl-10.5 font-medium ${isCurrent ? 'text-[#15803D] font-bold' : isDone ? 'text-zinc-700' : 'text-zinc-400'}`}>
                        {stepItem.sub}
                      </p>
                    </div>
                  );
